...
import { GetSettlementDataArgs, GetSettlementDataResponse } from "../app/types";

export type CreateSettlementsArgs = {
  amount: number;
  note?: string;
  paidByUserId: Id<"users">;
  recievedByUserId: Id<"users">;
  groupId?: Id<"groups">;
...
export const createSettlements = mutation({
  ...
  handler: async (
    ...
    if (args.groupId) {
      ...
      if (!isMember(args.paidByUserId) || !isMember(args.recievedByUserId)) {
    ...
    return await ctx.db.insert("settlements", {
      ...
      receivedByUserId: args.recievedByUserId,
